...
const powers = [18, 21, 19, 25, 28, 24, 30, 32, 27, 34, 37, 33, 29, 31, 35, 39, 36, 32, 28, 26, 30, 34, 38, 35, 31, 29, 26, 24, 23, 22];
...
function tariffAt(hour: number): { label: string; price: string; rate: number; tone: TariffTone } {
  if (hour >= 18 && hour <= 20) return { label: "Horário de Pico", price: "R$ 1,20/kWh", rate: 1.2, tone: "peak" };
  if ((hour >= 7 && hour <= 17) || hour === 21) return { label: "Horário Intermediário", price: "R$ 0,85/kWh", rate: 0.85, tone: "mid" };
...
function liveElapsedSeconds(charger: Charger, now: Date | null) {
  ...
  return charger.elapsedSeedSeconds + ((now.getTime() - charger.sessionStartedAt) / 1000) * SIMULATION_SPEED;
}
...
function liveSessionMetrics(charger: Charger, now: Date | null, rate: number) {
  ...
  const liveSeconds = charger.sessionStartedAt && now ? ((now.getTime() - charger.sessionStartedAt) / 1000) * SIMULATION_SPEED : 0;
...
function Panel({ children, className }: { children: ReactNode; className?: string }) {
  return <section className={cn("panel-glow rounded-xl border border-border bg-card", className)}>{children}</section>;
}

function Title({ icon: Icon, children, detail }: { icon: LucideIcon; children: ReactNode; detail?: string }) {
  return (
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    setNow(firstTick);
    setChargers((items) =>
      items.map((cp) => (cp.status === "Carregando" && !cp.sessionStartedAt ? { ...cp, sessionStartedAt: firstTick.getTime() } : cp)),
    );
  ...
  const totalPowerKw = chargers.reduce((sum, cp) => sum + (cp.status === "Carregando" ? cp.power : 0), 0);
  ...
  const networkLabel = networkLoad === 0 ? "Rede livre" : networkTone === "critical" ? "Carga crítica" : networkTone === "attention" ? "Operação em atenção" : "Operação estável";
  const networkClass = networkTone === "critical" ? "text-primary" : networkTone === "attention" ? "text-warning" : "text-success";
  const networkStroke = networkTone === "critical" ? "var(--chart-red)" : networkTone === "attention" ? "var(--chart-orange)" : "var(--chart-green)";
  const latestDemandLabel = now ? now.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : "Agora";
  ...
  const demandData = useMemo(
    () =>
      baseDemandData.map((point, index) =>
        index === baseDemandData.length - 1 ? { time: latestDemandLabel, power: Number(totalPowerKw.toFixed(1)) } : point,
      ),
  ...
  const badgeTone = tariff.tone === "peak" ? "border-primary/40 bg-primary/10 text-primary" : tariff.tone === "mid" ? "border-warning/40 bg-warning/10 text-warning" : "border-success/40 bg-success/10 text-success";

  const kpis: Array<{ label: string; value: string; color: string; icon: LucideIcon }> = [
    { label: "Potência em Uso", value: `${formatDecimal(totalPowerKw)} kW / ${NETWORK_LIMIT_KW} kW`, color: networkTone === "critical" ? "text-primary" : networkTone === "attention" ? "text-warning" : "text-success", icon: Bolt },
    { label: "Carregadores Ativos", value: `${activeCount}/${chargers.length}`, color: "text-success", icon: BatteryCharging },
    { label: "Receita Acumulada", value: formatMoney(totalRevenue), color: "text-info", icon: DollarSign },
  ...
  const impact: Array<{ value: string; label: string; color: string; icon: LucideIcon }> = [
    { value: `${formatDecimal(totalEnergy * CO2_KG_PER_KWH, 2)} kg`, label: "CO₂ Evitado Total", color: "text-success", icon: Leaf },
    { value: `${Math.round(totalEnergy * CLEAN_KM_PER_KWH)} km`, label: "Deslocamento Limpo Equivalente", color: "text-info", icon: CarFront },
    { value: formatMoney(totalEnergy * FOSSIL_SAVINGS_PER_KWH), label: "Economia vs Combustível Fóssil", color: "text-warning", icon: DollarSign },
    { value: String(completedSessions + activeCount), label: "Recargas Sustentáveis Realizadas", color: "text-violet", icon: BatteryCharging },
  ];
  ...
  return (
    <div className="grid-surface min-h-screen bg-background text-foreground">
        ...
        <div className="mx-auto grid max-w-[1600px] grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-4 lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden truncate text-xs text-muted-foreground sm:block">Sistema de Gestão Comercial de Eletropostos — FIAP × GoodWe 2026</p>
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <div className={cn("hidden items-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold md:flex", badgeTone)}>
              <DollarSign size={15} />
      ...
      <main className="mx-auto max-w-[1600px] space-y-5 px-4 py-5 lg:px-8 lg:py-7">
        <div className={cn("flex items-center justify-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold md:hidden", badgeTone)}>
          <DollarSign size={15} />
          {tariff.label} — {tariff.price}
        </div>
        ...
        <section className="grid gap-5 lg:grid-cols-5">
          <Panel className="min-w-0 lg:col-span-3">
            <Title icon={Activity} detail="Atualização contínua · últimos 30 pontos">Controle de Demanda em Tempo Real</Title>
            <div className="h-[300px] p-4">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={demandData} margin={{ top: 12, right: 10, left: -18, bottom: 0 }}>
                  ...
                  <YAxis domain={[0, 50]} unit=" kW" tick={{ fill: "var(--muted-foreground)", fontSize: 10 }} tickLine={false} axisLine={false} />
                  <Tooltip contentStyle={{ background: "var(--popover)", border: "1px solid var(--border)", borderRadius: "8px" }} formatter={(value) => [`${value} kW`, "Demanda"]} />
                  <ReferenceLine y={NETWORK_LIMIT_KW} stroke="var(--chart-orange)" strokeDasharray="6 5" label={{ value: "Limite da Rede", fill: "var(--chart-orange)", fontSize: 11, position: "insideTopRight" }} />
                  <Area type="monotone" dataKey="power" stroke="var(--chart-red)" strokeWidth={3} fill="url(#demandFill)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </Panel>
          <Panel className="lg:col-span-2">
            <Title icon={Gauge} detail={`${formatDecimal(networkLoad, 0)}% da capacidade`}>Status da Rede</Title>
            <div className="flex h-[300px] flex-col items-center justify-center px-6">
              <div className="relative w-full max-w-[320px]">
                <svg viewBox="0 0 220 125" role="img" aria-label={`${formatDecimal(totalPowerKw)} kW de ${NETWORK_LIMIT_KW} kW em uso`} className="w-full">
                  <path d="M 25 110 A 85 85 0 0 1 195 110" fill="none" stroke="var(--muted)" strokeWidth="18" strokeLinecap="round" pathLength="100" />
                  <path d="M 25 110 A 85 85 0 0 1 195 110" fill="none" stroke={networkStroke} strokeWidth="18" strokeLinecap="round" pathLength="100" strokeDasharray={`${networkLoad} 100`} />
                </svg>
                <div className="absolute inset-x-0 bottom-0 text-center">
                  <p className="font-display text-4xl font-bold">
                    {formatDecimal(totalPowerKw, 0)} <span className="text-lg text-muted-foreground">kW</span>
                  </p>
                  <p className={cn("mt-1 text-xs uppercase tracking-[.18em]", networkClass)}>{networkLabel}</p>
                </div>
              </div>
        ...
        <section>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {chargers.map((cp) => (
              <ChargerCard key={cp.id} charger={cp} now={now} rate={tariff.rate} onStart={() => startCharger(cp.id)} onStop={() => stopCharger(cp.id)} />
            ))}
          </div>
        </section>
        <section className="grid gap-5 xl:grid-cols-5">
            ...
            <div className="h-[280px] px-2 pt-5 sm:px-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={tariffData} margin={{ top: 12, right: 4, left: -24, bottom: 0 }}>
                  <CartesianGrid stroke="var(--grid-line)" strokeDasharray="3 5" vertical={false} />
                  <XAxis dataKey="hour" tick={{ fill: "var(--muted-foreground)", fontSize: 9 }} tickLine={false} axisLine={false} interval={2} />
                  <YAxis domain={[0, 1.5]} tick={{ fill: "var(--muted-foreground)", fontSize: 9 }} tickLine={false} axisLine={false} />
                  <Tooltip cursor={{ fill: "var(--muted)" }} contentStyle={{ background: "var(--popover)", border: "1px solid var(--border)", borderRadius: "8px" }} formatter={(value) => [`R$ ${Number(value).toFixed(2).replace(".", ",")}/kWh`, "Tarifa"]} />
                  <Bar dataKey="value" radius={[3, 3, 0, 0]}>
                    {tariffData.map((d) => (
                      <Cell key={d.hour} fill={d.tone === "red" ? "var(--chart-red)" : d.tone === "orange" ? "var(--chart-orange)" : "var(--chart-green)"} stroke={d.hour === hour ? "var(--foreground)" : "transparent"} strokeWidth={2} />
                    ))}
                  </Bar>
                  <ReferenceLine x={hour} stroke="var(--foreground)" label={{ value: "AGORA", fill: "var(--foreground)", fontSize: 9, position: "top" }} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          <Panel className="min-w-0 overflow-hidden xl:col-span-3">
            <Title icon={Clock3} detail="Últimas sessões encerradas">Histórico de Sessões</Title>
            <div className="max-h-[340px] overflow-auto">
              <table className="w-full min-w-[760px] text-left text-xs">
                <thead className="sticky top-0 z-10 bg-primary text-primary-foreground">
                  <tr>{["CP", "Veículo", "kWh", "Duração", "Tarifa", "Custo (R$)", "Encerrado em"].map((h) => <th key={h} className="px-4 py-3 font-semibold">{h}</th>)}</tr>
                </thead>
                <tbody>
                  {sessions.map((row, i) => (
                    <tr key={`${row[0]}-${row[6]}`} className={cn("border-b border-border", i % 2 ? "bg-panel-raised/60" : "bg-card")}>
                      {row.map((cell, j) => (
                        <td key={j} className={cn("px-4 py-3", j === 0 ? "font-display font-semibold text-primary" : "text-muted-foreground", j === 5 && "font-semibold text-foreground")}>{cell}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
        ...
        <Panel>
          <Title icon={Leaf} detail="Consolidado do mês">Impacto Ambiental — Recargas Realizadas</Title>
      ...
      <footer className="border-t border-border px-4 py-6 text-center text-xs text-muted-foreground">ChargeGrid Intelligence © 2026 — FIAP × GoodWe EV Challenge | Protótipo Acadêmico</footer>
    </div>
  );
}

function ChargerCard({ charger, now, rate, onStart, onStop }: { charger: Charger; now: Date | null; rate: number; onStart: () => void; onStop: () => void }) {
  ...
  const durationLabel = charging ? formatDuration(metrics.elapsedSeconds) : waiting ? "Na fila" : "—";

  return (
    <Panel className={cn("overflow-hidden transition-colors", charging && "border-success/35", waiting && "border-warning/35")}>
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 border-b border-border bg-panel-raised/50 px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          <Zap size={17} className={charging ? "text-success" : waiting ? "text-warning" : "text-muted-foreground"} />
          <h3 className="truncate font-display font-bold">{charger.id}</h3>
        </div>
        <span className={cn("rounded-full border px-2 py-1 text-[10px] font-semibold", charging ? "border-success/35 bg-success/10 text-success" : waiting ? "border-warning/35 bg-warning/10 text-warning" : "border-border bg-muted text-muted-foreground")}>{charger.status}</span>
      </div>
      <div className="space-y-3 p-4">
        ...
        <div>
          <div className="mb-1.5 flex justify-between text-xs">
            ...
            <span className={cn("font-display font-semibold", charging ? "text-success" : "text-muted-foreground")}>{formatDecimal(charger.power)} kW</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-muted">
            <div className={cn("h-full rounded-full transition-all duration-500", charging ? "bg-success" : waiting ? "bg-warning" : "bg-muted-foreground/30")} style={{ width: `${Math.min((charger.power / CHARGER_POWER_KW) * 100, 100)}%` }} />
          </div>
        </div>
